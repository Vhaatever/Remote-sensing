...
movingPoints=Refmap;
fixedPoints=Inputmap;
tform = images.geotrans.PolynomialTransformation2D(movingPoints,fixedPoints,2);
% Finds the coefficient matrix for Refrence map and input image

X=Refmap(:,1);
Y=Refmap(:,1);
U = tform.B(1) + tform.B(2).*X + tform.B(3).*Y + tform.B(4).*X.*Y + tform.B(5).*X.^2 + tform.B(6).*Y.^2

Outputnet=NMat(Refmap,TX,TY); % Multilies the transformation matrix and the Input matrix to get the output
